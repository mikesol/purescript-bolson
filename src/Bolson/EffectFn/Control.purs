module Bolson.EffectFn.Control
  ( flatten
...
import Bolson.EffectFn.Core (Child(..), DynamicChildren(..), Element(..), Entity(..), EventfulElement(..), FixedChildren(..), PSR, Scope(..))
import Control.Alt ((<|>))
...
import FRP.Event.EffectFn (Event, keepLatest, makeEvent, mapAccum, memoize, subscribe)
...
flatten
  ...
  DynamicChildren' (DynamicChildren children) ->
    makeEvent \(k :: payload -> Effect Unit) -> do
